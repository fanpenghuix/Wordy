...
CREATE TABLE sm2_reviews (
  ...
  last_review TEXT,                          -- Last review date
  last_correct INTEGER,                      -- Last quiz result (0 or 1)
...
| `last_correct` | Last quiz result (0=wrong, 1=correct) |
...
| Word deleted | Cascade delete sm2_reviews entry (ON DELETE CASCADE or app-level) |
